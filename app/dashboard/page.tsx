...
const MIGRATION_KEY = "beseder_migration_v1_done";
  ...
  useEffect(() => {
    ...
    loadBrandsFromCloud().then(cloud => {
      ...
      const migrated = (() => {
        ...
        const existingIds = new Set(base.map((b: Brand) => b.id));
        const toAdd = MIGRATION_BRANDS.filter(b => !existingIds.has(b.id));
        if (toAdd.length === 0) { localStorage.setItem(MIGRATION_KEY, "1"); return base; }
